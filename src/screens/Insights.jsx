...
function Insights() {
  ...
  const getPracticeHeatmap = useAnalyticsStore(
    (state) => state.getPracticeHeatmap,
  );
  const getMostPracticedPoses = useAnalyticsStore(
    (state) => state.getMostPracticedPoses,
  );
  const getFavoriteSessions = useAnalyticsStore(
    (state) => state.getFavoriteSessions,
  );
  const getTimeOfDayDistribution = useAnalyticsStore(
    (state) => state.getTimeOfDayDistribution,
  );
  ...
  const getAnalyticsSummary = useAnalyticsStore(
    (state) => state.getAnalyticsSummary,
  );

  // Analytics calculations - React Compiler handles memoization
  ...
  // Calculated insights - React Compiler handles memoization
  ...
  // Favorite time of day calculation - React Compiler handles memoization
  const favoriteTime = timeDistribution.reduce(
  ...
  if (totalSessions < 5) {
    return (
      <DefaultLayout
        header={
          <PageHeader title={t("screens.insights.title")} showBack={false} />
        }
      >
        <div className="flex flex-1 items-center justify-center px-4 py-8 text-center">
          <div className="mx-auto max-w-sm">
            ...
            <Text variant="secondary" className="mb-6">
              {t("screens.insights.unlockInsights", {
                count: 5 - totalSessions,
                plural: 5 - totalSessions !== 1 ? "s" : "",
              })}
            </Text>
      ...
      <ContentBody size="lg" spacing="lg" className="print:p-4">
        ...
        <div className="mb-6 grid w-full min-w-0 grid-cols-2 gap-2 sm:gap-3">
          ...
          <Stat
            ...
            icon={<Activity className="size-5" />}
            description={t("screens.insights.best", {
              count: summary.overall.longestStreak,
            })}
            variant="compact"
          />
        </div>
        ...
        {moodAnalytics.sessionsWithMoodData > 0 && (
          <div className="mb-8 grid w-full min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Stat
              ...
              description={t("screens.insights.sessionsImproved", {
                percent: moodAnalytics.improvementRate,
              })}
              variant="compact"
            />
            <Stat
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="space-y-2 text-sm">
                {practiceConsistency > 0.5 && (
                  <Text variant="body">
                    {t("screens.insights.consistencyMessage", {
                      avg: practiceConsistency,
                    })}
                  </Text>
                )}
                {favoriteTime.value > 0 && (
                  <Text variant="body">
                    {t("screens.insights.favoriteTimeMessage", {
                      time: favoriteTime.label.toLowerCase(),
                      count: favoriteTime.value,
                    })}
                  </Text>
                )}
                {mostPracticedPoses.length > 0 && (
                  <Text variant="body">
                    {t("screens.insights.topPoseMessage", {
                      pose: mostPracticedPoses[0].label,
                      count: mostPracticedPoses[0].value,
                    })}
                  </Text>
                )}
                {summary.overall.longestStreak >= 7 && (
                  <Text variant="body">
                    {t("screens.insights.longestStreakMessage", {
                      count: summary.overall.longestStreak,
                    })}
                  </Text>
                )}
                {moodAnalytics.averageMoodImprovement > 0 && (
                  <Text variant="body">
                    {t("screens.insights.moodImprovementMessage", {
                      improvement: moodAnalytics.averageMoodImprovement,
                    })}
                  </Text>
                )}
              </div>
        ...
        <div className="hidden border-t pt-4 text-center text-sm text-muted-foreground print:block">
          <Text variant="caption">
            Practice Insights • Generated on {new Date().toLocaleDateString()} •
