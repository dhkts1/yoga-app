/**
 * Sparkline Component - Linear/Notion Inspired
 *
 * Lightweight SVG sparkline for inline stats visualization.
 * Pure SVG rendering with no dependencies.
 */

import React from "react";
import PropTypes from "prop-types";
import cx from "classnames";

const Sparkline = React.memo(
  ({
    data = [],
    width = 80,
    height = 24,
    color = "currentColor",
    showArea = false,
    strokeWidth = 1.5,
    animate = true,
    className,
    ...props
  }) => {
    if (!data.length) return null;

    // Normalize data to fit within the SVG
    const padding = 2;
    const effectiveWidth = width - padding * 2;
    const effectiveHeight = height - padding * 2;

    const min = Math.min(...data);
    const max = Math.max(...data);
    const range = max - min || 1;

    // Generate points
    const points = data.map((value, index) => {
      const x = padding + (index / (data.length - 1 || 1)) * effectiveWidth;
      const y =
        padding + effectiveHeight - ((value - min) / range) * effectiveHeight;
      return { x, y };
    });

    // Create SVG path
    const linePath = points
      .map((point, i) => `${i === 0 ? "M" : "L"} ${point.x} ${point.y}`)
      .join(" ");

    // Create area path (for gradient fill)
    const areaPath = showArea
      ? `${linePath} L ${points[points.length - 1].x} ${height - padding} L ${padding} ${height - padding} Z`
      : null;

    return (
      <svg
        width={width}
        height={height}
        viewBox={`0 0 ${width} ${height}`}
        className={cx("overflow-visible", className)}
        {...props}
      >
        {/* Gradient definition */}
        {showArea && (
          <defs>
            <linearGradient
              id={`sparkline-gradient-${color}`}
              x1="0"
              y1="0"
              x2="0"
              y2="1"
            >
              <stop offset="0%" stopColor={color} stopOpacity="0.3" />
              <stop offset="100%" stopColor={color} stopOpacity="0" />
            </linearGradient>
          </defs>
        )}

        {/* Area fill */}
        {showArea && (
          <path
            d={areaPath}
            fill={`url(#sparkline-gradient-${color})`}
            className={animate ? "animate-fade-in" : ""}
          />
        )}

        {/* Line */}
        <path
          d={linePath}
          fill="none"
          stroke={color}
          strokeWidth={strokeWidth}
          strokeLinecap="round"
          strokeLinejoin="round"
          className={animate ? "animate-sparkline-draw" : ""}
          style={
            animate
              ? {
                  strokeDasharray: 100,
                  strokeDashoffset: 0,
                }
              : undefined
          }
        />

        {/* End dot */}
        <circle
          cx={points[points.length - 1].x}
          cy={points[points.length - 1].y}
          r={2}
          fill={color}
          className={animate ? "animate-scale-in" : ""}
        />
      </svg>
    );
  },
);

Sparkline.displayName = "Sparkline";

Sparkline.propTypes = {
  data: PropTypes.arrayOf(PropTypes.number),
  width: PropTypes.number,
  height: PropTypes.number,
  color: PropTypes.string,
  showArea: PropTypes.bool,
  strokeWidth: PropTypes.number,
  animate: PropTypes.bool,
  className: PropTypes.string,
};

export { Sparkline };
