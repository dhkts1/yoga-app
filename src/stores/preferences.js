    ...
    (set, get) => ({
      ...
      updateVoiceSettings: (settings) => {
        ...
        if (
          ...
          ["gentle", "motivational", "minimal"].includes(
            settings.voicePersonality,
          )
        ) {
        ...
        if (typeof settings.voiceSpeed === "number") {
          newSettings.voiceSpeed = Math.max(
            0.8,
            Math.min(1.2, settings.voiceSpeed),
          );
        }
        if (typeof settings.voiceVolume === "number") {
          newSettings.voiceVolume = Math.max(
            0.0,
            Math.min(1.0, settings.voiceVolume),
          );
        }
      ...
      setVoicePersonality: (personality) => {
        if (!["gentle", "motivational", "minimal"].includes(personality)) {
          console.warn(
            `Invalid voice personality: ${personality}. Using 'gentle' as default.`,
          );
          personality = "gentle";
        }
      ...
      setRestDuration: (duration) => {
        ...
        if (isNaN(numDuration) || numDuration < 0 || numDuration > 60) {
          console.warn(
            `Invalid rest duration: ${duration}. Using 5 as default.`,
          );
          duration = 5;
        } else {
      ...
      setDarkModeTheme: (darkTheme) => {
        if (!["slate", "taupe", "gray", "sage", "custom"].includes(darkTheme)) {
          console.warn(
            `Invalid dark mode theme: ${darkTheme}. Using 'slate' as default.`,
          );
          darkTheme = "slate";
        }
        set({ darkModeTheme: darkTheme });
        return darkTheme;
      },
      ...
      setCustomDarkColor: (color) => {
        ...
        if (!hexRegex.test(color)) {
          ...
          return get().customDarkColor;
        }
        set({ customDarkColor: color });
        return color;
      },
      ...
      dismissTooltip: (tooltipId) =>
        set((state) => ({
          tooltipsDismissed: [
            ...new Set([...state.tooltipsDismissed, tooltipId]),
          ],
        })),
      ...
      incrementTooltipShown: (tooltipId) =>
        set((state) => ({
          tooltipsShownCount: {
      ...
      setBreathingDefaultDuration: (duration) =>
        set((state) => ({
          breathing: {
            ...state.breathing,
      ...
      removeFavoriteSession: (sessionId) =>
        set((state) => ({
          favoriteSessions: state.favoriteSessions.filter(
            (id) => id !== sessionId,
          ),
        })),
      ...
      removeFavoriteExercise: (exerciseId) =>
        set((state) => ({
          favoriteExercises: state.favoriteExercises.filter(
            (id) => id !== exerciseId,
          ),
        })),
      ...
      migrate: (persistedState, version) => {
        ...
        if (version === 0) {
          ...
          return {
            ...
            darkModeTheme: persistedState.darkModeTheme ?? "slate",
            customDarkColor: persistedState.customDarkColor ?? "#282B2E",
