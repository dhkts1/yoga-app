...
test.describe("Keyboard Navigation", () => {
  ...
  test("should navigate through welcome screen interactive elements with Tab key", async ({
    page,
  }) => {
    ...
    const quickStartButton = page
      .getByRole("button", { name: /quick start|start/i })
      .first();
  ...
  test("should navigate through bottom navigation with Tab key", async ({
    page,
  }) => {
    await page.waitForLoadState("domcontentloaded");
    await page.waitForTimeout(500);
  ...
  test("should navigate bottom nav tabs with Enter key", async ({ page }) => {
    ...
    // Get Sessions tab button
    const sessionsTab = page.locator('nav button[aria-label="Sessions"]');
  ...
  test("should close mood tracker modal with Escape key", async ({ page }) => {
    ...
    const isVisible = await moodDialog
  ...
  test("should navigate through session list with Tab key", async ({
    page,
  }) => {
  ...
  test("should navigate through settings screen with Tab key", async ({
    page,
  }) => {
  ...
  test("should maintain logical tab order through practice flow", async ({
    page,
  }) => {
  ...
  test("should activate bottom nav tabs with keyboard and update aria-current", async ({
    page,
  }) => {
    await page.waitForLoadState("domcontentloaded");
    await page.waitForTimeout(500);
    ...
    // Get Sessions tab and activate it
    const sessionsTab = page.locator('nav button[aria-label="Sessions"]');
    await sessionsTab.focus();
    await page.keyboard.press("Enter");
    await page.waitForURL(/\/sessions/, { timeout: 3000 });

    // aria-current should move to Sessions tab
