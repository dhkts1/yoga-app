...
test.describe("Performance", () => {
  ...
  test("should handle 100+ completed sessions in practice history", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("text=/120|total sessions/i").first(),
    ).toBeVisible();

    // Should render streak correctly
    await expect(
      page.locator("text=/15.*day.*streak|streak.*15/i").first(),
    ...
    await expect(
      page.locator("text=/70.*minutes|7.*sessions/i").first(),
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test("should handle 50+ custom sessions without lag", async ({ page }) => {
    ...
    // Scroll should be smooth (no lag) - using touch for mobile
  ...
  test("should render Insights screen with large dataset under 3 seconds", async ({
    page,
  }) => {
    ...
    await page.evaluate(() => {
      const sessions = Array.from({ length: 100 }, (_, i) => ({
        id: `session-${i}`,
        sessionId: ["quick-start", "gentle-flow", "power-flow", "deep-stretch"][
          i % 4
        ],
        sessionName: [
          "Quick Start",
        ...
        poses: Array.from({ length: (i % 5) + 3 }, (_, j) => ({
          ...
          id: ["mountain-pose", "warrior-i", "downward-dog", "child-pose"][
            j % 4
          ],
        })),
        preMood: (i % 5) + 1,
    ...
    await expect(
      page.locator("text=/150|total.*sessions/i").first(),
    ...
    await expect(
      page.locator("text=/25.*day.*streak|streak.*25/i").first(),
    ).toBeVisible();
    await expect(page.locator("text=/1,?500|1500/i").first()).toBeVisible(); // Total minutes

    // Should be interactive (can scroll) - using touch for mobile
  ...
  test("should render Sessions screen with all sessions quickly", async ({
    page,
  }) => {
    ...
    // Should display session categories/tabs (if they exist)
    ...
    // Verify page is responsive (skip tab switching if tabs don't exist)
    const quickTab = page.getByRole("tab", { name: /quick|5.*min/i });
    if (await quickTab.isVisible({ timeout: 1000 }).catch(() => false)) {
  ...
  test("should keep localStorage under quota with large dataset", async ({
    page,
  }) => {
    ...
    await page.evaluate(() => {
      ...
      const sessions = Array.from({ length: 100 }, (_, i) => ({
        ...
        poses: Array.from({ length: 10 }, (_, j) => ({
          ...
          duration: 30,
          description:
            "A test pose for performance testing with enough data to simulate real usage",
          bodyParts: ["legs", "core"],
        })),
        preMood: (i % 5) + 1,
        postMood: (((i % 5) + 2) % 5) + 1,
      ...
      const breathing = Array.from({ length: 100 }, (_, i) => ({
        ...
        exerciseName: ["Box Breathing", "4-7-8 Breathing", "Alternate Nostril"][
          i % 3
        ],
        duration: [3, 5, 10][i % 3],
        completedAt: new Date(Date.now() - i * 86400000).toISOString(),
      ...
      const programProgress = {
        ...
        notes:
          "Making great progress with the program. Feeling stronger and more flexible.",
      };
      ...
      const preferences = {
        state: {
          ...
          favoriteSessions: Array.from(
            { length: 10 },
            (_, i) => `session-${i}`,
          ),
          favoriteExercises: Array.from(
            { length: 10 },
            (_, i) => `exercise-${i}`,
          ),
          theme: "light",
  ...
  test("should navigate quickly with large dataset", async ({ page }) => {
    ...
    const navigations = [
      ...
      },
      {
        ...
        action: async () => {
          await page
            .getByRole("button", { name: /insights/i, exact: true })
            .click();
        },
      },
      {
  ...
  test("should load session builder with maximum poses quickly", async ({
    ...
    await expect(
      page.getByRole("tab", { name: /select poses/i }).first(),
    ).toBeVisible();
    ...
    const sequenceItems = page
      .locator('[data-testid*="sequence-item"], [class*="sequence"]')
  ...
  test("should load program with all weeks completed without lag", async ({
    page,
  }) => {
        ...
        for (let day = 1; day <= sessionsPerWeek; day++) {
          sessions.push({
            ...
            completedAt: new Date(
              Date.now() -
                (totalWeeks - week) * 7 * 86400000 -
                (sessionsPerWeek - day) * 86400000,
            ).toISOString(),
            poses: [
              { nameEnglish: "Mountain Pose", id: "mountain-pose" },
              { nameEnglish: "Warrior I", id: "warrior-i" },
    ...
    const programCard = page
      .locator("text=/beginner.*foundation|foundation.*beginner/i")
      .first();
    await programCard.click();

    // Wait for program detail page
    await page.waitForURL(/\/programs\/beginner-foundations/, {
      timeout: 3000,
    });
    await page.waitForLoadState("networkidle");
    ...
    await expect(
      page.locator("text=/completed|finished|done|100%/i").first(),
    ).toBeVisible({ timeout: 5000 });
    ...
    await expect(
      page.locator("text=/completed|7.*\/.*7|100%/i").first(),
    ).toBeVisible({ timeout: 5000 });
  });
});
