...
test.describe("Multi-tab Synchronization", () => {
  ...
  test("should sync completed session and streak across tabs", async ({
    browser,
  }) => {
    ...
    try {
      ...
      await expect(
        page1.getByRole("button", { name: /quick start/i }),
      ).toBeVisible();
      await expect(
        page2.getByRole("button", { name: /quick start/i }),
      ).toBeVisible();
      ...
      const homeButton1 = page1
        .getByRole("link", { name: /home/i })
        .or(page1.getByRole("button", { name: /home|done/i }));
      await homeButton1.click();
      await page1.waitForURL("/");

      // Verify streak appears in tab 1 (displayed as "1d" format)
      await expect(page1.locator("text=/1d|1.*day/i")).toBeVisible();
      ...
      // Verify streak appears in tab 2 after reload (displayed as "1d" format)
      await expect(page2.locator("text=/1d|1.*day/i")).toBeVisible({
        timeout: 5000,
      });
      ...
      expect(storage1?.state?.totalSessions).toBe(
        storage2?.state?.totalSessions,
      );
      expect(storage1?.state?.currentStreak).toBe(
        storage2?.state?.currentStreak,
      );
    } finally {
  ...
  test("should sync theme toggle across tabs", async ({ browser }) => {
    ...
    try {
      ...
      // Navigate to settings in both tabs (Settings is in header with aria-label="Settings")
  ...
  test("should sync voice settings changes across tabs", async ({
    browser,
  }) => {
    const context1 = await browser.newContext();
    const context2 = await browser.newContext();
  ...
  test("should sync custom session creation across tabs", async ({
    browser,
  }) => {
    const context1 = await browser.newContext();
    const context2 = await browser.newContext();
    ...
    try {
      ...
      const builderButton = page1.getByRole("link", {
        name: /create.*own|custom.*session|session.*builder/i,
      });
      if (await builderButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const sessionNameInput = page1.getByPlaceholder(
          /session.*name|name.*session/i,
        );
        if (
          ...
          const addPosesTab = page1.getByRole("button", {
            name: /add.*poses|poses/i,
          });
          if (
            await addPosesTab.isVisible({ timeout: 2000 }).catch(() => false)
          ) {
            await addPosesTab.click();
            await page1.waitForTimeout(200);

            // Click first pose card to add it
            const firstPose = page1
              .locator("[data-pose-card], .pose-card, button")
              .filter({
            ...
            if (
              await firstPose.isVisible({ timeout: 2000 }).catch(() => false)
            ) {
              await firstPose.click();
              await page1.waitForTimeout(200);

              // Save the session
              const saveButton = page1.getByRole("button", {
                name: /save.*session|create.*session/i,
              });
              if (
                ...
                const customSession = storage1.find(
                  (s) => s.name === "Multi-Tab Test Session",
                );
                ...
                const syncedSession = storage2.find(
                  (s) => s.name === "Multi-Tab Test Session",
                );
  ...
  test("should sync custom session deletion across tabs", async ({
    browser,
  }) => {
    const context1 = await browser.newContext();
    const context2 = await browser.newContext();
    ...
    try {
      ...
      if (await deleteButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        await page2.getByRole("button", { name: /sessions/i }).click();
        await page2.waitForURL(/\/sessions/);
  ...
  test("should sync program enrollment status across tabs", async ({
    browser,
  }) => {
    const context1 = await browser.newContext();
    const context2 = await browser.newContext();
    ...
    try {
      ...
      const programsLink = page1
        .getByRole("link", { name: /programs/i })
        .or(page1.getByRole("button", { name: /programs/i }));
      ...
      if (await programsLink.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const firstProgram = page1
          .locator("[data-program-card], .program-card, button")
          .filter({
        ...
        if (
          await firstProgram.isVisible({ timeout: 2000 }).catch(() => false)
        ) {
          await firstProgram.click();
          await page1.waitForURL(/\/programs\/[^/]+$/);

          // Click "Start Program" button
          const startButton = page1.getByRole("button", {
            name: /start.*program|begin.*program/i,
          });
          if (
            await startButton.isVisible({ timeout: 2000 }).catch(() => false)
          ) {
            ...
            if (storage1?.state?.activeProgram) {
              ...
              const programsLink2 = page2
                .getByRole("link", { name: /programs/i })
                .or(page2.getByRole("button", { name: /programs/i }));
              ...
              if (
                ...
                const storage2 = await page2.evaluate(() => {
                  const programs = localStorage.getItem(
                    "yoga-program-progress",
                  );
                  return programs ? JSON.parse(programs) : null;
                });

                expect(storage2?.state?.activeProgram).toBe(
                  storage1.state.activeProgram,
                );
              }
            }
          }
  ...
  test("should sync favorite session toggle across tabs", async ({
    browser,
  }) => {
    ...
    try {
      ...
      const firstSession = page1
        .locator("[data-session-card], .session-card, button")
        .filter({
      ...
      if (await firstSession.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const favoriteButton = page1
          .getByRole("button", { name: /favorite|heart/i })
          .or(page1.locator("button").filter({ hasText: /♡|♥|★|☆/ }))
          .first();

        if (
          await favoriteButton.isVisible({ timeout: 2000 }).catch(() => false)
        ) {
          ...
          const newFavorites1 = await page1.evaluate(() => {
            const prefs = localStorage.getItem("mindful-yoga-preferences");
            return prefs
              ? JSON.parse(prefs)?.state?.favoriteSessions || []
              : [];
          });
  ...
  test("should handle simultaneous changes from both tabs gracefully", async ({
    browser,
  }) => {
    const context1 = await browser.newContext();
    const context2 = await browser.newContext();
    ...
    try {
      ...
      if (
        (await toggle1.isVisible().catch(() => false)) &&
        (await toggle2.isVisible().catch(() => false))
      ) {
        // Click both at roughly the same time
        await Promise.all([toggle1.click(), toggle2.click()]);
