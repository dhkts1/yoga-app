...
test.describe("Focus Management", () => {
  ...
  test("should focus main heading after route change (Welcome → Sessions)", async ({
    page,
  }) => {
  ...
  test("should focus main heading after route change (Sessions → Practice)", async ({
    page,
  }) => {
    // Navigate to Sessions
    await page.getByRole("button", { name: "Sessions", exact: true }).click();
  ...
  test("should trap focus in mood tracker modal", async ({ page }) => {
    ...
    const moodDialog = page
      .locator('[role="dialog"]')
      .filter({ hasText: /how are you feeling/i });
  ...
  test("should trap focus with Shift+Tab in mood tracker modal", async ({
    page,
  }) => {
    // Start practice to trigger mood tracker
    await page.getByRole("button", { name: /quick start/i }).click();
  ...
  test("should trap focus in confirmation dialog", async ({ page }) => {
    ...
    const clearButton = page
      .getByRole("button", { name: /clear.*history|reset/i })
      .first();
    ...
    if (await clearButton.isVisible({ timeout: 2000 })) {
      ...
      const confirmDialog = page
        .locator('[role="dialog"]')
        .filter({ hasText: /clear|confirm|delete/i });
  ...
  test("should skip link be visible when focused and functional", async ({
    page,
  }) => {
    ...
    const skipLinkVisible = await page.evaluate(() => {
      ...
      return (
        styles.clip !== "rect(0px, 0px, 0px, 0px)" && styles.width !== "1px"
      );
    });
  ...
  test("should return focus to trigger after closing mood tracker with Skip", async ({
    page,
  }) => {
    // Start practice to trigger mood tracker
    await page.getByRole("button", { name: /quick start/i }).click();
  ...
  test("should return focus to trigger after closing confirmation dialog with Escape", async ({
    page,
  }) => {
  ...
  test("should maintain focus during breathing practice interactions", async ({
    page,
  }) => {
    ...
    const exerciseCard = page
      .locator('article, div[class*="card"]')
      .filter({ hasText: /box|4-7-8/i })
      .first();
  ...
  test("should prevent focus from leaving modal when tabbing", async ({
    page,
  }) => {
    // Start practice to trigger mood tracker
    await page.getByRole("button", { name: /quick start/i }).click();
  ...
  test("should focus first element when modal opens", async ({ page }) => {
    ...
    const focusInfo = await page.evaluate(() => {
      ...
      return {
        ...
        tag: el?.tagName || "",
        isInteractive: ["BUTTON", "A", "INPUT", "TEXTAREA", "SELECT"].includes(
          el?.tagName || "",
        ),
      };
    });
