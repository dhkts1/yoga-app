...
test.describe("Session Browsing", () => {
  ...
  test("should navigate to Sessions tab and show sessions", async ({
    page,
  }) => {
    // Navigate to Sessions/Sessions tab - using button in bottom nav (exact match to avoid "Discover Multi-Week Programs")
  ...
  test("should show category tabs on Sessions screen", async ({ page }) => {
    // Navigate to Sessions - using button in bottom nav (exact match to avoid "Discover Multi-Week Programs")
    const discoverTab = page.getByRole("button", {
      name: "Sessions",
      exact: true,
    });
  ...
  test("should filter sessions when category is clicked", async ({ page }) => {
    ...
    const sessionCards = page
      .locator("button")
      .filter({
        hasText: /morning energizer|lunch break|evening wind-down|quick reset/i,
      });
  ...
  test("should navigate to session preview when session card is clicked", async ({
    page,
  }) => {
    // Navigate to Sessions - using button in bottom nav (exact match to avoid "Discover Multi-Week Programs")
    const discoverTab = page.getByRole("button", {
  ...
  test("should show session details on preview screen", async ({ page }) => {
    ...
    const sessionName = page.getByRole("heading", {
      name: /morning energizer/i,
    });
  ...
  test("should allow favoriting a session", async ({ page }) => {
    ...
    const favoriteButton = page.getByRole("button", {
      name: /add morning-energizer to favorites/i,
    });
