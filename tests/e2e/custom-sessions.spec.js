...
test.describe("Custom Session Creation", () => {
  ...
  test("should create a custom session with multiple poses", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/build your custom yoga sequence/i),
    ).toBeVisible();
    ...
    const poseCards = page
      ...
      .filter({ hasText: /mountain|warrior|tree|child|downward|cobra/i });
    for (let i = 0; i < 3; i++) {
    ...
    await expect(
      page.getByRole("tab", { name: /selected poses.*3/i }),
    ).toBeVisible();
  ...
  test("should show validation error when saving without poses", async ({
    page,
  }) => {
  ...
  test("should practice with custom session", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Quick Test Session|pose/i }).first(),
    ).toBeVisible();
  });
  ...
  test("should show custom sessions in session list", async ({ page }) => {
    ...
    await expect(page.getByText("My Custom Flow")).toBeVisible({
      timeout: 3000,
    });
  });
  ...
  test("should clear all poses when clicking clear button", async ({
    page,
  }) => {
    await page.goto("/sessions/builder");
    ...
    await expect(
      page.getByRole("heading", { name: /clear.*session/i }),
    ).toBeVisible();
  ...
  test("should show empty state when no poses added", async ({ page }) => {
    ...
    await expect(page.getByText(/no poses yet/i)).toBeVisible();
    await expect(page.getByText(/switch to.*select poses/i)).toBeVisible();
  });
  ...
  test("should persist draft when navigating away", async ({ page }) => {
    ...
    await expect(
      page.getByRole("tab", { name: /selected poses.*1/i }),
    ).toBeVisible();
  });

  test("should create custom session and complete practice end-to-end", async ({
    page,
  }) => {
    // Ensure clean state
    await dismissOnboardingIfPresent(page);
    ...
    // Switch to Select Poses tab and select 2 poses
    const addPosesTab = page.getByRole("tab", { name: /select poses/i });
    await addPosesTab.click();
    await page.waitForTimeout(200);
