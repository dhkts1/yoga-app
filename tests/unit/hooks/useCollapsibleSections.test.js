...
describe("useCollapsibleSections", () => {
  ...
  describe("Combined operations", () => {
    ...
    test("should handle openAll then closeAll", () => {
      ...
      expect(
        Object.values(result.current.openSections).every((v) => v === true),
      ).toBe(true);
      ...
      expect(
        Object.values(result.current.openSections).every((v) => v === false),
      ).toBe(true);
    });
  ...
  describe("Edge cases", () => {
    ...
    test("should handle null initial state gracefully", () => {
      ...
      // Implementation defaults to {} when null is passed (useState(initialState) with null becomes {})
      // This is expected behavior - the hook normalizes null to an empty object
      expect(result.current.openSections).toEqual({});
      ...
      // After toggle, section should be true
      expect(result.current.openSections.section1).toBe(true);
    });
  });
