...
describe('MoodTracker', () => {
  it('renders mood selection interface', () => {
    ...
    expect(screen.getByText(/This helps us understand your starting point/i)).toBeInTheDocument();
  });
  ...
  it('allows selecting mood and energy levels', async () => {
    ...
    await waitFor(() => {
      expect(onComplete).toHaveBeenCalledWith({ mood: expect.any(Object), energy: expect.any(Object) });
    }, { timeout: 500 });
  });
  ...
  it('shows post-practice messaging when isPostPractice is true', () => {
    ...
    render(<MoodTracker onComplete={onComplete} onSkip={onSkip} isPostPractice={true} />);

    expect(screen.getByText(/Let's see how your practice went/i)).toBeInTheDocument();
  });
  ...
  it('shows progress indicators', () => {
    ...
    const progressDots = screen.getByRole('dialog').querySelectorAll('.rounded-full.w-2.h-2');
