...
vi.mock('../../../src/components/design-system', () => ({
  ...
  CircularProgress: ({ value, className }) => (
    <div className={className} data-progress={value}>Progress: {value}%</div>
  ),
}));
...
describe('BreathingGuide', () => {
  ...
  it('shows correct instructions for inhale phase', () => {
    ...
    expect(screen.getByText(/Breathe in slowly through your nose/i)).toBeInTheDocument();
  });
  ...
  it('resets to inhale phase when not active', () => {
  ...
  it('returns null when exercise is not provided', () => {
    const { container } = render(<BreathingGuide exercise={null} isActive={true} />);
  ...
  it('handles rapid phase transitions correctly', async () => {
    ...
    // Quickly advance through all phases
